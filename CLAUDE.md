...
# Run tests + linting (default rake task)
bundle exec rake
...
4. **Session Management**:
   - Boots Rails via `config/environment.rb` on first session request
...
- **Type System**: Sorbet strict mode. All code must include `sig` blocks with type signatures.
- **Target Ruby**: 3.1+
- **Linting**: RuboCop with performance, rake, rspec, and sorbet plugins
